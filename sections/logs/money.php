...
if (!in_array($sortby, array("updatetime", "name", "uid", "cashdiff", "bankdiff", "moneydiff")))
    $sortby = "updatetime";
...
if (!empty($searchstr)) {
    $List = searchUpdateLog($searchstr, $sortby, $order, $start, $pagesize, $count);
...
$pg->totalrecords = $count != -1 ? $count : getUpdateLogDBCount();
$pg->showfirst = true;
$pg->showlast = true;
$pg->paginationcss = "pagination-large";
$pg->paginationstyle = 1; // 1: advance, 0: normal
$pg->defaultUrl = "index.php?page=logs&amp;action=money&amp;sortby=".$sortby."&amp;order=".$order.$searchparam;
$pg->paginationUrl = "index.php?page=logs&amp;action=money&amp;sortby=".$sortby."&amp;order=".$order."&amp;pagenum=[p]".$searchparam;

// Get Players (if not a search request)
if (empty($searchparam)) {
    $List = getUpdateLogEntries($sortby, $order, $start, $pagesize);
}
